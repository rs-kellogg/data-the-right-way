...
# -----------------------------------------------------------------------------
def run_query(client, database, template, arg_list, test_run):
    args = arg_list.pop(0)
    sql = template.render(args)
    console.print(f"STARTED: {args}")
    if test_run:
        return args
    else:
        return args, client.start_query_execution(
            QueryString=sql, 
            QueryExecutionContext={
                'Database':database
            }
...
def run_queries(
    ...
    # check for valid parameters
    assert len(args_list) <= 1000
    assert batch_size <= 100

    remaining_args: List = args_list.copy()
    results: List = []
    running: List = []

    # initialize running jobs up to batch size or number of jobs requested
    for i in range(min(batch_size, len(remaining_args))):
        running.append(run_query(client, database, template, remaining_args, test_run))

    # keep adding jobs until no more are left
    while len(remaining_args) > 0:
        if len(running) < batch_size:
            for i in range(min(len(remaining_args), batch_size - len(running))):
                running.append(run_query(client, database, template, remaining_args, test_run))
        remove_completed_jobs(client, running, results, test_run)
